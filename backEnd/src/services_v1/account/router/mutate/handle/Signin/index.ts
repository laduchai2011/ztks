            ...
            if (result && result?.length > 0) {
                ...
                switch (device) {
                    case DeviceEnum.WEB: {
                        ...
                        const myJwtPayload: My_Jwt_Payload_Field = {
                            id: id,
                        };
                        ...
                        const access_token = generate_access_token(myJwtPayload, signOptions_accessToken);
                        const refresh_token = generate_refresh_token(myJwtPayload, signOptions_refreshToken);
                        const socket_token = generate_socket_token(myJwtPayload, signOptions_socketToken);
                        ...
                        if (resultget?.isSuccess) {
                            ...
                            const resultset = await mssqlSetValue(keyServiceRedisWeb, JSON.stringify(storeAuthToken));
                        ...
                        await serviceRedis.setData<StoreAuthToken>(keyServiceRedisWeb, storeAuthToken, timeExpireat);
                    ...
                    case DeviceEnum.MOBILE: {
                        ...
                        await serviceRedis.setData<StoreAuthToken>(keyServiceRedisMobile, storeAuthToken, timeExpireat);
