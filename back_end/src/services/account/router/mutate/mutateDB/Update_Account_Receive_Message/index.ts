...
class MutateDB_Update_Account_Receive_Message {
    ...
    set_Update_Account_Receive_Message_Body(
        update_account_receive_message_body: Update_Account_Receive_Message_Body_Field
    ): void {
        ...
        if (this._update_account_receive_message_body !== undefined) {
            ...
            try {
                ...
                const result = await pool.query<Account_Receive_Message_Field>(
                    `SELECT * FROM update_account_receive_message($1, $2, $3);`,
                    [
