...
class MutateDB_Create_Account_Receive_Message {
    ...
    set_Create_Account_Receive_Message_Body(create_account_receive_message_body: Create_Account_Receive_Message_Body_Field): void {
    ...
    async run(): Promise<Account_Receive_Message_Field | undefined> {
        if (this._create_account_receive_message_body !== undefined) {
            ...
            try {
                const account_id_receive_message =
                ...
                const result = await pool.query<Account_Receive_Message_Field>(`SELECT * FROM create_account_receive_message($1, $2, $3);`, [
