...
class QueryDB_Get_Account_Receive_Message {
    ...
    set_Get_Account_Receive_Message_Body(get_account_receive_message_body: Get_Account_Receive_Message_Body_Field): void {
    ...
    async run(): Promise<Account_Receive_Message_Field | void> {
        if (this._get_account_receive_message_body !== undefined) {
            try {
                const result = await pool.query<Account_Receive_Message_Field>(`SELECT * FROM get_account_receive_message($1, $2);`, [
