...
class QueryDB_Get_List_Chat_Room_Phones {
    ...
    async run(): Promise<Chat_Room_Phone_Field[] | void> {
        if (this._get_list_chat_room_phones_body !== undefined) {
            try {
                const result = await pool.query<Chat_Room_Phone_Field>(`SELECT * FROM get_list_chat_room_phones($1, $2);`, [
                    this._get_list_chat_room_phones_body.chat_room_id,
                    this._get_list_chat_room_phones_body.account_id   
                ]);
