...
class Handle_Upload_A_Video_To_Tks_Store {
    ...
    upload_Chunk = async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (!Number.isInteger(index) || index < 0) {
                res.status(400).json({ message: 'Invalid chunk_index' });
                return;
            }

            const file_path = req.file.path;
            const object_name = `chunks/${file_id}/${index}`;

            const stream = fs.createReadStream(file_path);

            const result = await minioService.uploadStream(object_name, stream, req.file.size, req.file.mimetype);
...
export default Handle_Upload_A_Video_To_Tks_Store;
