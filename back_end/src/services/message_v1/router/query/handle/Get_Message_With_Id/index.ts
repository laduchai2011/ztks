...
import { get_Message_With_Id } from '../../queryMongo/Get_Message_With_Id';

class Handle_Get_Message_With_Id {
    main = async (req: Request<any, any, any, { id: string }>, res: Response) => {
        ...
        const my_response: My_Response_Field<Message_V1_Field<Zalo_Message_Type>> = {
            is_success: false,
            message: 'Bắt đầu (Handle_Get_Message_With_Id-main)',
        };

        const result = await get_Message_With_Id(id);
...
export default Handle_Get_Message_With_Id;
